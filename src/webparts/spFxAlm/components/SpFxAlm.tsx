...
export default function SpFxAlm(props: ISpFxAlmProps) {
  ...
  const [isSiteAppCatalog, setSiteAppCatalog] = React.useState(false);
  const [file, setFile] = React.useState(null);
  const [hideDialog, hideDialogBox] = React.useState(true);
  const [operationInfo, setMessage] = React.useState("");
  const [operationCompleted, setoperationStatus] = React.useState(false);

  const [siteURL, setSiteURL] = React.useState("");
  const [allSiteURL, setAllSiteURL] = React.useState([]);
  const [tentantAppURL, setTenantAppURL] = React.useState("");

  const [allSPFxApps, setAllApps] = React.useState([]);
  const [appID, setAppID] = React.useState(null);

  const [showaddremoveError, setShowUploadError] = React.useState({ addAppSelectSiteErr: false, addAppSelectAppErr: false, uploadAppSelectAppErr: false });
  const [showDeployRetractError, setShoweployRetractError] = React.useState({ deployAppSelectSiteErr: false, deployAppSelectAppErr: false });
  const [showunInstallError, setshowunInstallError] = React.useState({ appinstallSelectSiteErr: false, appinstallSelectAppErr: false });
